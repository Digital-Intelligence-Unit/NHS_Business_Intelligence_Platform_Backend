...
pool.then((pgPool) => {
    ...
    const directoryPath = "./postgres/backup_data/";
    readFiles(directoryPath, function (fileName) {
        ...
        fs.createReadStream(filePath)
            ...
            .on("end", function () {
                ...
                if (csvData.length) {
                    ...
                    // this can be used to determine how many rows will be inserted at once
                    let counter = 0;
                    ...
                    csvData.forEach((row) => {
                        // If the counter has no value then this is the first row of the csv file,
                        // so we create the start of the insert query
    ...
    function updateTable(arrQuery) {
        ...
        if (arrQuery.length) {
            let counter = 0;
            arrQuery.forEach((objQuery) => {
                pool.query(objQuery, (error, results) => {
                    if (error) {
    ...
    function readFiles(dirname, onFileContent, onError) {
        fs.readdir(dirname, function (err, filenames) {
            ...
            filenames.forEach(function (filename) {
                fs.readFile(dirname + filename, "utf-8", function (errFiles, content) {
                    if (errFiles) {
                        onError(errFiles);
                        return;
                    }
